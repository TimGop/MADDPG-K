...
class MARL_TRAINER(object):
    def __init__(self, gamma, tau, env, good_agent_network, adv_agent_network, lr, num_adv, num_good, agent_list,
        ...
        self.obs_sizes = [0]
        self.as_ag = env.action_space(agent_list[0]).shape[0]
        self.obs_sizes = [0]
        for agent in agent_list:
            self.obs_sizes.append(self.obs_sizes[-1] + env.observation_space(agent).shape[0])
        self.knn_obs = [0]
        for agent in range(num_adv_obs):
            self.knn_obs.append(self.knn_obs[-1] + env.observation_space(agent_list[agent]).shape[0])
        for agent in range(num_good_obs):
            self.knn_obs.append(self.knn_obs[-1] + env.observation_space(agent_list[agent+num_adv]).shape[0])
            
        
    def get_obs_inds(self,array):
        vec_array_f = np.vectorize(self.get_obs_i,signature='(n) -> (l)')
        r = vec_array_f(array)
        return r
    
    def get_obs_i(self, array):
        lst = []
        for a in array:
            lst.append(np.arange(self.obs_sizes[a],self.obs_sizes[a+1]))
        return np.concatenate(lst)
    
    def get_act_inds(self,array):
        vec_array_f = np.vectorize(self.get_act_i,signature='(n) -> (l)')
        r = vec_array_f(array)
        return r
    
    def get_act_i(self, array):
        lst = []
        for i,a in enumerate(array):
            lst.append(np.arange(self.as_ag*i,self.as_ag*(i+1)))
        return np.concatenate(lst)
    ...
    def update(self, memory, agent_list, agent, agent_indices, BATCH_SIZE):
        ...
        # collect replay sample from all agents
            ...
            for agent_id in agent_list:
                ...
                act_n.append(act)
            np_obs_n = torch.cat(obs_n, dim = 1)
            np_obs_next_n = torch.cat(obs_next_n, dim = 1)
            np_act_n = torch.cat(act_n, dim = 1)
            knn_indices = None
            if self.kNN_enabled:
                ...
                lsts_np = np.array(knn_obs_lsts)
                lsts_next_np = np.array(knn_obs_nxt_lsts)
                obs_inds = torch.tensor(self.get_obs_inds(lsts_np))
                obs_next_inds = torch.tensor(self.get_obs_inds(lsts_next_np))
                act_inds = torch.tensor(self.get_act_inds(lsts_np))
                act_inds_e = torch.ones_like(act_inds) * torch.arange(BATCH_SIZE).unsqueeze(1)
                obs_inds_e = torch.ones_like(obs_inds) * torch.arange(BATCH_SIZE).unsqueeze(1)
                np_obs_n = np_obs_n[obs_inds_e,obs_inds]
                np_act_n = np_act_n[act_inds_e,act_inds]
                np_obs_next_n = np_obs_next_n[obs_inds_e,obs_next_inds]
                ...
                for knn_obs_lst in knn_obs_lsts:
                    ...
                    btch_idx += 1
                """
                #Still needs vectorizing 
                knn_indices = {agent_list[agent_id]: i for i,agent_id in
                           enumerate(knn_obs_lsts[0])}
                target_act_next_n = [[] for _ in range(len(knn_indices))]
                btch_idx = 0
                for batch_id,knn_obs_nxt_lst in enumerate(knn_obs_nxt_lsts):
                    knn_nxt_indices = {agent_list[agent_id]: i for i,agent_id in
                           enumerate(knn_obs_nxt_lsts[batch_id])}
                    for i, agent_id in enumerate(knn_obs_nxt_lst):
                ...
                obs_next_n = []
                act_n = []
                for i in range(len(knn_obs_lsts[0])):
                    obs_n.append(np_obs_n[:,self.knn_obs[i]:self.knn_obs[i+1]])
                    obs_next_n.append(np_obs_next_n[:,self.knn_obs[i]:self.knn_obs[i+1]])
                    act_n.append(np_act_n[:,self.as_ag*i: self.as_ag*(i+1)])
