...
def parse_args():
    ...
    parser.add_argument("--num-good", type=int, default=7, help="number of agents")
    ...
    parser.add_argument("--bootstrap", type=bool, default=False, help="starting training with random sampling")
    ...
    parser.add_argument("--central", type=bool, default=False, help="all agents share a combined critic and actor during training")
    # Benchmarking
...
def get_knn(obs, agent, agent_list, num_good_obs, num_adv_obs, good_start):
    knn_list = []
    #agent_is_adv = agent.__contains__("adversary")
    ag_id = agent_list.index(agent)
    dist = np.stack(list(obs.values()))[:,2:4]
    dist = np.linalg.norm(dist - dist[ag_id], axis= 1) + 1
    dist[ag_id] = 0
    adv_dist_ind = []
    if num_adv_obs > 0:
        adv_dist_ind = dist[:good_start].argsort(axis=0)[:num_adv_obs]
    good_dist_ind = dist[good_start:].argsort(axis=0)[:num_good_obs]
    """# ORDER OF LIST:
    # [ ...adv(potentially include agent_curr at beginning)..., ...ag(potentially include agent_curr at beginning)...]
    adv_dist = []
    good_dist = []
    coord_agent = obs[agent][2:4]
    for i, ag in enumerate(agent_list):
        if ag == agent and agent_is_adv:
            adv_dist.append(0)
            continue
    ...
    good_dist_ind = good_dist.argsort(axis=0)[:num_good_obs] + num_adv_obs"""
...
def display(env, BATCH_SIZE, lr, gamma, n_episodes, good_agent_network, adv_agent_network, update_iter, save_iter,
            ...
            bootstrap_sampling, eps, comb_crit, wd, grad_clip, num_good_obs, num_adv_obs, kNN_enabled, central):
    ...
    for i_episode in range(n_episodes):
        while True:
            ...
            for agent in agent_list:
                ...
                if kNN_enabled:
                    knn_obs_lst = get_knn(obs_n, agent, agent_list, num_good_obs, num_adv_obs, good_start)
                    knn_obs_nxt_lst = get_knn(new_obs_n, agent, agent_list, num_good_obs, num_adv_obs, good_start)
...
if __name__ == '__main__':
    ...
    if args.kNN_enabled and args.adv_agent == "maddpg":
        ...
        for s in parallel_env.possible_agents:
            if s.__contains__("agent") and n_good_obs > 0:
                ...
                n_good_obs -= 1
            if s.__contains__("adversary") and n_adv_obs > 0:
                sum_act_size += parallel_env.action_space(s).shape[0]
                sum_obs_size += parallel_env.observation_space(s).shape[0]
                n_adv_obs -= 1
    else:
    ...
    if args.display:
        display(env=parallel_env, BATCH_SIZE=args.batch_size, lr=args.lr, gamma=args.gamma, tau=args.tau,
                ...
                num_good_obs=args.num_good_obs, num_adv_obs=args.num_adv_obs, kNN_enabled=args.kNN_enabled, central = args.central)
    else:
