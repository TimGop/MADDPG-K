...
def get_agents(n_adv, n_good, agent_list, gamma, tau, lr, env, adv_agent_network, good_agent_network, adv_model, good_model, comb_crit, wd):
    agents = {}
    
    adv_critic = Critic(adv_agent_network["critic_input_size"], hidden=adv_agent_network["actor_n_hidden"]).to(device)
    good_critic = Critic(good_agent_network["critic_input_size"], hidden=good_agent_network["actor_n_hidden"]).to(device)
    ...
    for i in range(n_good):
        agents.update({agent_list[i + n_adv]: good_model(gamma=gamma, tau=tau, env=env,
            ...
            hidden=good_agent_network["actor_n_hidden"], critic = adv_critic, wd=wd)})
        if comb_crit:
            agents.update({agent_list[i + n_adv]: good_model(gamma=gamma, tau=tau, env=env,
            in_features=good_agent_network["actor_input_size"],
            in_features_Q=good_agent_network["critic_input_size"], lr=lr,
            hidden=good_agent_network["actor_n_hidden"], critic = good_critic)})
    return agents
...
class MARL_TRAINER(object):
    ...
    def update(self, memory, agent_list, agent, agent_indices, BATCH_SIZE):
        ...
        # collect replay sample from all agents
            ...
            self.update_MADDPG(rew=rew, done=done, obs_n=obs_n, obs_next_n=obs_next_n, act_n=act_n, agent_list=agent_list, agent=agent, agent_indices=agent_indices)
